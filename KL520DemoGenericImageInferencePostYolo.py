...
IMAGE_FILE_PATH = os.path.join(PWD, '../../res/images/car.png')
...
class_list = [
    ...
    "chair", "sofa", "potted plant", "bed", "dining table", "toilet", "tv monitor", "laptop", "mouse", "remote",
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KL520 Demo Generic Image Inference with YOLO Post-Process Example.')
    ...
    for yolo_box_result in yolo_result.box_list:
        ...
        color = (b, g, r)
        class_name=class_list[yolo_box_result.class_num]
        text=class_name
        cv2.rectangle(img=img, pt1=(int(yolo_box_result.x1), int(yolo_box_result.y1)), pt2=(int(yolo_box_result.x2), int(yolo_box_result.y2)), color=color)
        
        cv2.putText(img, text, (int(yolo_box_result.x1), int(yolo_box_result.y1) - 10),cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
